...
class MeanSquaredErrorLoss():
    def forward(self, y_real, y_pred):
        m = y_real.shape[0]
        return (1/(2*m)) * np.sum((y_pred - y_real)**2)  # la somma dell'errore di tutti i neuroni k di output
        # uso axis=0 nel caso di categorical loss
    def backward(self, y_real, y_pred):
        m = y_real.shape[0]
        return (1/m) * np.sum((y_pred - y_real))
...
class Binary_CrossEntropyLoss():
    def forward(self, y_pred, y_real):
        n = y_real.shape[0]
        return (-1 / n) * np.sum((y_real * np.log(y_pred) + (1 - y_real) * np.log(1 - y_pred)))
    def backward(self, y_pred, y_real):
